...
CREATE TABLE  CATEGORIES  (
   CATEGORY_CODE       NUMERIC(6)                       NOT NULL,
...
/*==============================================================*/
CREATE TABLE            ORDERS  (
   ORDER_NUMBER           NUMERIC(6)                       NOT NULL,
   CUSTOMER_CODE          VARCHAR(5)                         NOT NULL,
   ...
   FOREIGN KEY (CUSTOMER_CODE) REFERENCES CUSTOMERS (CUSTOMER_CODE) ON DELETE NO ACTION ON UPDATE NO ACTION
    );
...
CREATE TABLE            ORDER_DETAILS  (
   ORDER_NUMBER         NUMERIC(6)                       NOT NULL,
   PRODUCT_REF         NUMERIC(6)                       NOT NULL,
...
CREATE TABLE            SUPPLIERS  (
   SUPPLIER_NUMBER     NUMERIC(6)                       NOT NULL,
   COMPANY             VARCHAR(40)                   NOT NULL,
...
CREATE TABLE            PRODUCTS (
   ...
   QUANTITY            VARCHAR(30),
   UNIT_PRICE       	NUMERIC(8,2)                     NOT NULL,
   UNITS_IN_STOCK        NUMERIC(5),
   UNITS_ON_ORDER   	NUMERIC(5),
   UNAVAILABLE	        NUMERIC(1),
   CONSTRAINT PK_PRODUCTS  PRIMARY KEY (PRODUCT_REF),
   FOREIGN KEY (SUPPLIER_NUMBER) REFERENCES SUPPLIERS (SUPPLIER_NUMBER) ON DELETE NO ACTION ON UPDATE NO ACTION
  );
/*==============================================================*/
...
INSERT INTO PRODUCTS VALUES (15, 'Genen Shouyu', 6, 2, '24 bottles (250 ml)', 78.00, NULL,NULL, NULL);
...
/*Ajouter une clonne Sexe à la table des employees
-----------------------------------------------------*/
...
UPDATE EMPLOYEES 
SET SEXE = 'F'
WHERE( FIRST_NAME = 'Nancy' or  FIRST_NAME = 'Anne'or  FIRST_NAME = 'Janet'or FIRST_NAME = 'Margaret');

UPDATE EMPLOYEES 
SET SEXE = 'M'
WHERE( FIRST_NAME = 'Andrew' or  FIRST_NAME = 'Steven'or FIRST_NAME = 'Michael' or FIRST_NAME ='Robert');

/* creation d'une clonne age et Hire et calcule de l'age et de l'encienneté
---------------------------------------------------------------------------
---------------------------------------------------------------------------*/
...
UPDATE EMPLOYEES
 SET HIRE  = DATEDIFF (YEAR, HIRE_DATE, GETDATE());


 /*INSTRUCTION 3 */

SELECT EMPLOYEE_NUMBER,LAST_NAME, FIRST_NAME, SEXE, AGE, HIRE
FROM EMPLOYEES  
 WHERE SEXE= 'M' 
 ...
 FROM PRODUCTS
   WHERE QUANTITY LIKE '%bottles%' and  CHARINDEX ('t', PRODUCT_NAME) = 3 
   and  SUPPLIER_NUMBER IN (1, 2, 3)
   and (UNIT_PRICE BETWEEN 70 and 200 ) and UNITS_ON_ORDER IS not null 

   /* Afficher les clients qui résident dans la même region*/
   ...
   /*QUESTION 6 Afficher le taux de remise*/

SELECT 
    ORDER_NUMBER,
    CASE 
        WHEN ORDER_NUMBER BETWEEN 0 AND 2000 THEN 0
        WHEN ORDER_NUMBER BETWEEN 2001 AND 10000 THEN 0.05
        WHEN ORDER_NUMBER BETWEEN 10001 AND 40000 THEN 0.10
        WHEN ORDER_NUMBER BETWEEN 40001 AND 80000 THEN 0.15
        ELSE 0.20
...
WHERE 
	...
	/* Question 7 Afficher les fournisseurs de boisson
	--------------------------------------------------*/

	SELECT SUPPLIER_NUMBER, COMPANY, ADDRESS_,  PHONE
	FROM SUPPLIERS
	...
	/* Question 8 : Affichez les clients de Berlin qui ont commandé au maximum 1 (0 ou 1) produit de dessert
	--------------------------------------------------------------------------------------------------------*/
